...
void NeochunkManager::_ready() {
    // Подписываемся на изменения в SemanticWorld.
    // Используем call_deferred, чтобы гарантированно обработать это в главном потоке
    // и безопасно работать с деревом чанков (roots).
...
void NeochunkManager::_on_shape_changed(uint64_t shape_id, const AABB& bounds) {
    // Расширяем AABB на небольшой запас (например, 2 единицы), 
    // чтобы гарантированно захватить чанки, граничащие с изменённой фигурой
